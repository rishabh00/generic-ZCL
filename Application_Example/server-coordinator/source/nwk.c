...
void ( *DeviceRemove_Handler )( uint16_t ID );
/************************************************************************************************
 * Function prototype
 ************************************************************************************************/
 ...
 * Function definition
 *****************************************************************************************************/
void NW_RegisterSapHandlers(void (*MCPS_ZCL_SapHandler)( NwkMsg_t msg ), void ( *DeviceRemove )( uint16_t ID ))
{
    /* MCPS_ZCL_SapHandler(); */
    MCPS_ZCL_Handler = MCPS_ZCL_SapHandler;
	DeviceRemove_Handler = DeviceRemove;
} 
...
void RemoveDeviceFromList( int client_sock )
{
    ...
    while (  ptr != NULL )
    {
        if( ptr->client_sid  == client_sock )
        {
			...
			printf("Device id which is removed:%"PRIu16"\n", device_ID);
			DeviceRemove_Handler( device_ID );
        }
        tmp = ptr;
        ptr = ptr->next;
    }       
