...
void ( *APME_DeivceRemove_Handler )( uint16_t );
void die(char *s);
...
void APS_RegisterSapHandlers(void (*APME_ZDO_SapHandler)( DataIndication_t MsgDecode ), void (*APDE_ZCL_SapHandler)( DataIndication_t MsgDecode ), void (*DeviceRemove)( uint16_t ID ) )
{
	...
	APME_DeivceRemove_Handler = DeviceRemove;
} 
...
{
	APME_DeivceRemove_Handler( device_ID );
}
	
/********** Initialization of Application support sub layer ***********/
void APS_init()
{
	NW_RegisterSapHandlers( &MCPS_ZCL_SapHandler,&DeviceRemove );
}
